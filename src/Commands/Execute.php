...
class Execute extends Console\Command\Command
{
	...
	protected function configure(): void
	{
		$this
			->setName(self::NAME)
			->setDescription('FB MQTT connector service')
			->setDefinition(
	...
	protected function execute(Input\InputInterface $input, Output\OutputInterface $output): int
	{
		...
		$io->title($this->translator->translate('//fb-mqtt-connector.cmd.execute.title'));

		$io->note($this->translator->translate('//fb-mqtt-connector.cmd.execute.subtitle'));
		...
		if (
			...
			if ($connector === null) {
				$io->warning(
					$this->translator->translate('//fb-mqtt-connector.cmd.execute.messages.connector.notFound'),
				);
			...
			if (count($connectors) === 0) {
				$io->warning($this->translator->translate('//fb-mqtt-connector.cmd.execute.messages.noConnectors'));
			...
			if (count($connectors) === 1) {
				...
				if ($input->getOption('no-interaction') === false) {
					$question = new Console\Question\ConfirmationQuestion(
						$this->translator->translate(
							'//fb-mqtt-connector.cmd.execute.questions.execute',
				...
				$question = new Console\Question\ChoiceQuestion(
					$this->translator->translate('//fb-mqtt-connector.cmd.execute.questions.select.connector'),
				...
				$question->setValidator(
					function (string|int|null $answer) use ($connectors): Entities\FbMqttConnector {
						if ($answer === null) {
		...
		if (!$connector->isEnabled()) {
			$io->warning($this->translator->translate('//fb-mqtt-connector.cmd.execute.messages.connector.disabled'));
		...
		if ($result !== Console\Command\Command::SUCCESS) {
			$io->error($this->translator->translate('//fb-mqtt-connector.cmd.execute.messages.error'));
